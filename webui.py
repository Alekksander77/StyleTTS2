...
def ui_load(config, ckpt, lang):
    global _current_model
    _current_model = TTSModel(config, ckpt, language=lang)
    return "Model loaded"
...
def build_ui():
    with gr.Blocks() as demo:
        ...
        with gr.Tab("Inference"):
            ...
            lang = gr.Textbox(value="fr-fr", label="Language")
            load_btn = gr.Button("Load")
            load_out = gr.Textbox(label="Status")
            load_btn.click(ui_load, [cfg_inf, ckpt, lang], load_out)
